...
class Email_Automation
{
    ...
    private const OPTION = 'bde_email_templates';
    ...
    private const ATTACHMENT_SLUGS = ['confirmacion_plaza'];
    ...
    public function __construct()
    {
        ...
        add_action('deleted_post_meta', [$this, 'clear_panel_cache'], 10, 3);
    }
    ...
    {
        if ($meta_key === '_bde_panel_page') {
            delete_option('bde_panel_reservas_page_id');
        }
    }
            ...
            'recepcion' => [
                ...
                'body' => '<h1>Hola {nombre},</h1>'
                    ...
                    . Email_Layout::meta_table([
                        ...
                        ['label' => 'Aportación',  'value' => '{importe}'],
                        ['label' => 'Reserva',     'value' => '{codigo_reserva}'],
                    ])
    ...
    {
        $actividad_id = (int) get_post_meta($inscripcion_id, '_bde_actividad_id', true);
        if (!$actividad_id) {
            return false;
        }
    ...
    public function cron_reminder_7dias(): void
    {
        ...
        $activities = $this->get_activities_for_reminder('reminder_7dias', $target_start, $target_end, '_bde_fecha_inicio');
    ...
    public function cron_reminder_24h(): void
    {
        ...
        $activities = $this->get_activities_for_reminder('reminder_1dia', $target_start, $target_end, '_bde_fecha_inicio');
    ...
    public function cron_reminder_1hora(): void
    {
        ...
        $activities = $this->get_activities_for_reminder('reminder_1hora', $target_start, $target_end, '_bde_fecha_inicio');
    ...
    public function cron_feedback(): void
    {
        ...
        $activities = $this->get_activities_for_reminder('reminder_post_evento', $target_start, $target_end, '_bde_fecha_fin');
    ...
    private function send(string $slug, int $inscripcion_id, int $actividad_id): void
    {
        ...
        $html_body = Email_Layout::render($body, $subject, [
        ...
        update_post_meta($inscripcion_id, '_bde_last_contact', wp_date('Y-m-d H:i:s'));
        update_post_meta($inscripcion_id, '_bde_last_contact_type', 'email');
        update_post_meta($inscripcion_id, '_bde_last_email_sent', $slug);
    ...
    {
        return Email_Layout::render($body, $subject, [
            'footer_text' => __('Has recibido este email porque estás inscrito en una de nuestras actividades.', 'convoca-enroll'),
        ]);
    }
    ...
    private function build_vars(int $inscripcion_id, int $actividad_id): array
    {
        ...
        if (!$panel_page_id) {
            $panel_page = get_pages(['meta_key' => '_bde_panel_page', 'meta_value' => '1', 'number' => 1]);
            if (!empty($panel_page)) {
                $panel_page_id = $panel_page[0]->ID;
                update_option('bde_panel_reservas_page_id', $panel_page_id);
            }
        }
