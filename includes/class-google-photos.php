...
class Google_Photos
{
    ...
    public function create_album(int $actividad_id): ?array
    {
        ...
        $album_id = get_post_meta($actividad_id, '_bde_google_album_id', true);
        if (!empty($album_id)) {
            return [
                'id' => $album_id,
                'url' => get_post_meta($actividad_id, '_bde_google_album_url', true),
            ];
        }
        ...
        $fecha = get_post_meta($actividad_id, '_bde_fecha_inicio', true);
        ...
        try {
            ...
            update_post_meta($actividad_id, '_bde_google_album_id', $created->getId());
            update_post_meta($actividad_id, '_bde_google_album_created_at', current_time('mysql'));
    ...
    public function share_album(int $actividad_id): ?string
    {
        ...
        $already_shared = get_post_meta($actividad_id, '_bde_google_album_shared', true);
        if ($already_shared) {
            return get_post_meta($actividad_id, '_bde_google_album_url', true);
        }
        ...
        try {
            ...
            update_post_meta($actividad_id, '_bde_google_album_url', $shareable_url);
            update_post_meta($actividad_id, '_bde_google_album_shared', '1');
    ...
    public function get_participants_emails(int $actividad_id): array
    {
        ...
        foreach ($inscriptions as $insc) {
            $email = get_post_meta($insc->ID, '_bde_email', true);
    ...
    public static function cron_share_albums(): void
    {
        ...
        $activities = get_posts([
            ...
            'meta_query' => [
                ...
                [
                    'key' => '_bde_google_album_id',
                    'compare' => 'EXISTS',
                ],
                [
                    'key' => '_bde_google_album_shared',
                    'compare' => 'NOT EXISTS',
                ],
            ],
        ]);
        ...
        foreach ($activities as $activity) {
            $create_option = get_post_meta($activity->ID, '_bde_google_create_album', true);
