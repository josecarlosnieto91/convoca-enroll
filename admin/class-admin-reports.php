...
class Admin_Reports
{
    ...
    public static function render(): void
    {
        ...
        <div class="wrap bde-reports-page">
            ...
            <nav class="nav-tab-wrapper">
                <?php foreach ($instance->tabs as $id => $label) : 
                    $active = ($instance->current_tab === $id) ? 'nav-tab-active' : '';
                    $url = add_query_arg(['tab' => $id], admin_url('admin.php?page=bde-informes'));
                ?>
    ...
    private function get_waitlist_data(): array
    {
        ...
        foreach ($activity_ids as $act_id) {
            ...
            if ($counts['lista_espera'] === 0) continue;

            $promoted = $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->postmeta} pm
                 JOIN {$wpdb->posts} p ON p.ID = pm.post_id
                 WHERE pm.meta_key = '_bde_actividad_id' AND pm.meta_value = %d
    ...
    private function tab_financiero(): void
    {
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                ...
                <?php else : foreach ($financials as $fin) : $total_general += $fin['total']; ?>
                    <tr>
    ...
    private function get_financial_data(): array
    {
        ...
        foreach ($activity_ids as $act_id) {
            $ingresos = $wpdb->get_results($wpdb->prepare(
            ...
            if (empty($ingresos)) continue;

            $fin = ['id' => $act_id, 'title' => get_the_title($act_id), 'total' => 0, 'tarjeta' => 0, 'bizum' => 0];
            foreach ($ingresos as $row) {
                ...
                $v = $raw / 100;
                $fin['total'] += $v;
                if ($row->method === 'tarjeta') $fin['tarjeta'] += $v;
                if ($row->method === 'bizum') $fin['bizum'] += $v;
            }

            // Calculate pending.
            $fin['pendiente'] = (float) $wpdb->get_var($wpdb->prepare(
    ...
    private function get_monitor_audit_data(): array
    {
        ...
        foreach ($results as $row) {
            ...
            if (strpos($row->message, 'Confirmad') !== false) $grouped[$key]['confirmadas']++;
            if (strpos($row->message, 'Cancelad') !== false) $grouped[$key]['canceladas']++;
            if (strpos($row->message, 'Check-in') !== false) $grouped[$key]['checkins']++;
        }
    ...
    private function tab_memoria(): void
    {
        ...
        <div class="bde-report-filters">
            <form method="get" action="" class="bde-filter-form">
                ...
                <div class="bde-filter-actions">
                    ...
                    <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['export' => 'memoria', 'year' => $year], admin_url('admin.php?page=bde-informes&tab=memoria')), 'bde_export_csv')); ?>" class="button">Exportar Memoria CSV</a>
                </div>
            </form>
        </div>
        ...
        <table class="wp-list-table widefat fixed striped">
            ...
            <tbody>
                ...
                <?php else : foreach ($memoria as $row) : ?>
                    <tr>
    ...
    private function get_memoria_data(int $year): array
    {
        ...
        foreach ($posts as $post) {
            ...
            foreach($resp_ids as $uid) {
                ...
                if($u) $resp_names[] = $u->display_name;
            }
    ...
    private function tab_evaluaciones(): void
    {
        ...
        <div class="bde-mb-4" style="display:flex;gap:10px;align-items:center;flex-wrap:wrap;">
            ...
            <a href="<?php echo esc_url(wp_nonce_url(add_query_arg(['export' => 'evaluaciones', 'filter_actividad' => $filter_actividad ?: '']), 'bde_export_csv')); ?>" class="button"><?php esc_html_e('Exportar CSV', 'convoca-enroll'); ?></a>
        </div>
        <table class="wp-list-table widefat fixed striped">
            <thead>
                <tr>
    ...
    public function handle_export(): void
    {
        ...
        switch ($type) {
            case 'ocupacion':
                fputcsv($out, ['Actividad', 'Fecha', 'Plazas Totales', 'Ocupadas', 'Disponibles', 'Asistentes'], ';');
                $data = $this->get_occupancy_data(wp_date('Y-01-01'), wp_date('Y-12-31'), 'todas');
                foreach ($data as $r) fputcsv($out, [$r['title'], $r['fecha'], $r['plazas_totales'], $r['confirmadas'], $r['plazas_disponibles'], $r['asistentes']], ';');
                break;
            case 'espera':
                fputcsv($out, ['Actividad', 'En espera', 'Ratio promocion (%)'], ';');
                $data = $this->get_waitlist_data();
                foreach ($data as $r) fputcsv($out, [$r['title'], $r['en_espera'], $r['ratio_promocion']], ';');
                break;
            case 'financiero':
                fputcsv($out, ['Actividad', 'Total (€)', 'Tarjeta (€)', 'Bizum (€)', 'Pendiente (€)'], ';');
                $data = $this->get_financial_data();
                foreach ($data as $r) fputcsv($out, [$r['title'], $r['total'], $r['tarjeta'], $r['bizum'], $r['pendiente']], ';');
                break;
            case 'memoria':
                $year = isset($_GET['year']) ? (int) $_GET['year'] : (int) wp_date('Y');
                fputcsv($out, ['Actividad', 'Fecha', 'Responsables', 'Participantes', 'Socios/as', 'Ubicación'], ';');
                $data = $this->get_memoria_data($year);
                foreach ($data as $r) fputcsv($out, [$r['title'], $r['fecha'], $r['responsables'], $r['participantes'], $r['socios'], $r['ubicacion']], ';');
                break;
            case 'evaluaciones':
                $filter_act = isset($_GET['filter_actividad']) ? (int) $_GET['filter_actividad'] : 0;
                fputcsv($out, ['ID', 'Actividad', 'Usuario', 'Fecha', 'Gestion', 'Instalaciones', 'Participantes', 'Comunicacion', 'Comentarios Gestion', 'Necesidades', 'Mejoras Gestion', 'Mejoras Instalaciones', 'Comentarios Participantes', 'Aspectos Positivos', 'Aspectos a Mejorar', 'Otros Comentarios'], ';');
                $batch = 0;
                $per_batch = 200;
                do {
                    ...
                    $eval_ids = $wpdb->get_col($wpdb->prepare(
                        ...
                        array_merge($meta_args, [$per_batch, $batch * $per_batch])
                    ));
                    if (empty($eval_ids)) break;
