...
class Admin_Page
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_bde_change_state', [$this, 'ajax_change_state']);
        add_action('wp_ajax_bde_toggle_checkin', [$this, 'ajax_toggle_checkin']);
        add_action('wp_ajax_bde_resend_email', [$this, 'ajax_resend_email']);
        add_action( 'admin_notices', [ $this, 'notices' ] );
        add_filter( 'parent_file', [ $this, 'fix_menu_highlight' ] );
        add_filter( 'submenu_file', [ $this, 'fix_submenu_highlight' ] );
        add_action('admin_post_bde_export_inscripciones_pdf', [$this, 'handle_export_inscripciones_pdf']);
        add_action('admin_post_bde_retry_email', [$this, 'handle_retry_email']);
        add_action('wp_ajax_bde_save_nota', [$this, 'ajax_save_nota']);
    }
    ...
    public function notices(): void
    {
        ...
        $messages = [
            'bde_date_error' => ['message' => __('Error: La fecha de fin no puede ser anterior a la de inicio. Se ha ajustado automáticamente.', 'convoca-enroll'), 'type' => 'danger'],
            'bde_google_photos_error' => ['message' => __('Google Photos: La integración no está configurada o activada. Se ha desactivado la creación del álbum.', 'convoca-enroll'), 'type' => 'warning'],
            'bde_google_calendar_error' => ['message' => __('Google Calendar: La integración no está configurada o activada. Se ha desactivado la sincronización.', 'convoca-enroll'), 'type' => 'warning'],
            'bde_plazas_adjusted' => ['message' => __('Aviso: Las plazas disponibles no pueden superar las totales. Se han ajustado automáticamente.', 'convoca-enroll'), 'type' => 'warning'],
        ];
    ...
    public function add_menu(): void
    {
        ...
        add_submenu_page(
            ...
            [Admin_Inscripcion_Form::class, 'render']
        );
    ...
    public function assets(string $hook): void
    {
        ...
        wp_enqueue_style('bde-admin', BDE_URL . 'assets/css/biodevas-enroll-admin.css', [], BDE_VERSION);
        wp_enqueue_script('bde-admin', BDE_URL . 'assets/js/biodevas-enroll-admin.js', ['biodevas-common-admin-js'], BDE_VERSION, true);
        wp_localize_script('bde-admin', 'bdeAdmin', [
        ...
        if (isset($_GET['page']) && $_GET['page'] === 'convoca-enroll') {
            wp_enqueue_script('bde-admin-js', BDE_URL . 'assets/js/biodevas-enroll-admin.js', ['biodevas-common-admin-js'], BDE_VERSION, true);
        }
        ...
        if ($hook === 'actividad_page_bde-monitor-crm') {
            wp_enqueue_script('bde-crm', BDE_URL . 'assets/js/biodevas-enroll-crm.js', ['biodevas-common-admin-js'], BDE_VERSION, true);
            wp_localize_script('bde-crm', 'bdeCrm', [
                'ajaxUrl' => admin_url('admin-ajax.php'),
    ...
    {
        wp_add_dashboard_widget('bde_stats', __('Inscripciones Biodevas', 'convoca-enroll'), [$this, 'render_widget']);
    }
    ...
    public function render_delegados(): void
    {
        ...
        if (isset($_POST['bde_save_delegados']) && check_admin_referer('bde_save_delegados')) {
            $assignments = (array) ($_POST['delegados'] ?? []);
            update_option('bde_delegados_actividades', $assignments);
            echo '<div class="updated"><p>Asignaciones guardadas.</p></div>';
        }

        $delegados = get_users(['role__in' => ['administrator', 'editor', 'author']]);
        $actividades = CPT_Actividad::get_upcoming(50);
        $current_assignments = (array) get_option('bde_delegados_actividades', []);
        ...
        foreach ($delegados as $user) {
            $user_id = $user->ID;
            $assigned = (array) ($current_assignments[$user_id] ?? []);
    ...
    private function handle_bulk_actions(): void
    {
        ...
        $ids = array_map('intval', (array) ($_GET['inscripcion'] ?? []));
        if (empty($ids)) {
            return;
        }
        ...
        foreach ($ids as $id) {
            ...
            if (!in_array($estado, ['pendiente', 'pendiente_pago'], true)) {
                $skipped_state++;
                continue;
            }
        ...
        if ($skipped_capacity > 0 || $skipped_state > 0) {
            $message .= ' ' . sprintf(
                __('(%d saltadas por falta de aforo, %d por estado no confirmable).', 'convoca-enroll'),
                $skipped_capacity,
                $skipped_state
            );
        }
    ...
    public function handle_export_inscripciones_pdf(): void
    {
        ...
        $headers = [__('Nombre', 'convoca-enroll'), __('Email', 'convoca-enroll'), __('Estado', 'convoca-enroll'), __('Actividad', 'convoca-enroll')];
        $rows = [];
    ...
    public function handle_retry_email(): void
    {
        ...
        $wpdb->update($table, ['status' => 'pending', 'retries' => 0, 'next_retry_at' => null], ['id' => $id]);
