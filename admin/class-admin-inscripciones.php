...
class Inscriptions_List extends \WP_List_Table
{
    ...
    protected function extra_tablenav($which): void
    {
        ...
        // Actividad filter.
        $acts_args = ['post_type' => 'actividad', 'posts_per_page' => -1, 'post_status' => 'publish'];
    ...
    public function prepare_items(): void
    {
        ...
        if (null !== $allowed_ids) {
            ...
            $args['meta_query'] = array_merge(['relation' => 'AND'], $existing_mq, [
                ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => $allowed_ids, 'compare' => 'IN'],
            ]);
        }
        ...
        if ($search) {
            ...
            $clean_search = strtoupper(str_replace([' ', '-'], '', $search));
            $args['s'] = $search;
            $search_mq = [
                'relation' => 'OR',
            ];
            ...
            if (is_email($search)) {
                $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => $search, 'compare' => '='];
            } else {
                $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => $search, 'compare' => 'LIKE'];
            }
            
            // Use exact match for DNI (contains numbers, no @)
            if (preg_match('/^\d{7,8}[A-Z]$/i', $clean_search)) {
                $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'dni', 'value' => $clean_search, 'compare' => '='];
            } else {
                $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'dni', 'value' => $search, 'compare' => 'LIKE'];
                $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'dni', 'value' => $clean_search, 'compare' => 'LIKE'];
            }
            
            $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'nombre', 'value' => $search, 'compare' => 'LIKE'];
            $search_mq[] = ['key' => CPT_Inscripcion::META_PREFIX . 'telefono', 'value' => $search, 'compare' => 'LIKE'];
            $existing_mq = $args['meta_query'] ?? [];
            $args['meta_query'] = array_merge(['relation' => 'AND'], $existing_mq, [$search_mq]);
        }

        // Filters.
        $meta_query = [];
        if (!empty($_GET['estado_filter'])) {
            $meta_query[] = ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => sanitize_text_field($_GET['estado_filter'])];
        }
        if (!empty($_GET['actividad_filter'])) {
            $meta_query[] = ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => (int) $_GET['actividad_filter']];
        }
        ...
        if ($meta_query) {
            ...
            $args['meta_query'] = array_merge(['relation' => 'AND'], $existing, $meta_query);
        }
    ...
    public function column_nombre($item): string
    {
        ...
        if ($member_id) {
            $actions['card'] = '<a href="' . esc_url(wp_nonce_url(admin_url('admin-post.php?action=bdv_pdf_card&member_id=' . $member_id), 'bdv_pdf_card_' . $member_id)) . '" target="_blank">🪪 ' . esc_html__('Tarjeta Socio', 'convoca-enroll') . '</a>';
        }
    ...
    public function column_asistencia($item): string
    {
        ...
        if ($status === 'si') {
            return '<span class="biodevas-badge biodevas-badge--success">Asistió</span>';
        }
        if ($status === 'no') {
            return '<span class="biodevas-badge biodevas-badge--error">No asistió</span>';
        }
        return '<span class="biodevas-badge biodevas-badge--info">Sin registrar</span>';
    }
    ...
    public function column_doc_voluntario($item): string
    {
        ...
        if ($doc_id) {
            $url = get_post_meta($doc_id, '_bdv_documento_url', true);
            if ($url) {
        ...
        if ($user) {
            if (in_array('voluntario_aprobado', (array) $user->roles) || get_user_meta($user->ID, '_bdv_es_voluntario', true)) {
                $is_voluntario = true;
            }
        }
