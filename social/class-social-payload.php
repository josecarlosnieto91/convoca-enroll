...
class Social_Payload {
	...
	private static function get_meta_location( int $post_id ): string {
		return get_post_meta( $post_id, '_convoca_ubicacion', true ) ?: get_post_meta( $post_id, '_convoca_ubicacion', true ) ?: '';
	}

	private static function get_meta_price( int $post_id ): string {
		$p = get_post_meta( $post_id, '_convoca_precio_socio', true ) ?: get_post_meta( $post_id, '_convoca_precio_socio', true );
		return $p ? "{$p}€" : ( self::is_free( $post_id ) ? 'Gratuito' : '' );
	}

	private static function is_free( int $post_id ): bool {
		$p = get_post_meta( $post_id, '_convoca_precio_socio', true ) ?: get_post_meta( $post_id, '_convoca_precio_social', true );
	...
	private static function get_badge_text( int $post_id ): string {
		$tipo = get_post_meta( $post_id, '_convoca_tipo_actividad', true ) ?: get_post_meta( $post_id, '_convoca_tipo_actividad', true );
