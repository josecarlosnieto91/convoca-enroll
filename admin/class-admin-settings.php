...
class Admin_Settings
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_bde_preview_email', [$this, 'ajax_preview']);
    }
        ...
        <div class="biodevas-field">
            ...
            <?php if (isset($_GET['maint_res'])): 
                ...
                <div class="biodevas-alert biodevas-alert--warning" style="margin-top:10px;">
                    ...
                    <ul>
                        <li><?php echo sprintf(esc_html__('Huérfanos: %s', 'convoca-enroll'), esc_html($res['orphans'])); ?></li>
                        <li><?php echo sprintf(esc_html__('Descuadres de plazas: %s', 'convoca-enroll'), esc_html($res['capacity_mismatch'])); ?></li>
                    </ul>
...
<p>Al realizar la inscripción aceptas el tratamiento de tus datos personales para la gestión de la actividad y autorizas la posible toma de contenido gráfico durante el desarrollo de los talleres, destinado a la comunicación y difusión de Biodevas.</p>';
        ...
        <?php
        wp_editor($normas, 'bde_normas_inscripcion', [
    ...
    private static function render_tab_emails(array $templates): void
    {
        ...
        <?php foreach (Email_Automation::TEMPLATES as $slug):
            $tpl = $templates[$slug] ?? ['subject' => '', 'body' => '', 'attachment_id' => ''];
        ...
        <table class="form-table">
            <?php $settings = get_option(self::OPTION, []); ?>
            <tr>
...
<script>
    ...
    private static function render_tab_eval_reminders(array $s): void
    {
        ...
        $eval_reminders = $settings['eval_reminder'] ?? [];
    ...
    public function maybe_save(): void
    {
        ...
        if ($tab === 'eval_reminders') {
            $eval = $bde['eval_reminder'] ?? [];
            $settings['eval_reminder'] = [
