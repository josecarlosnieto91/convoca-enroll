...
if ( ! function_exists( 'wp_salt' ) ) :
	...
	function wp_salt( $scheme = 'auth' ) {
		...
		if ( defined( 'SECRET_KEY' ) && SECRET_KEY && ( !isset($duplicated_keys[SECRET_KEY]) || empty($duplicated_keys[SECRET_KEY]) ) ) {
			$values['key'] = SECRET_KEY;
		}
		if ( 'auth' === $scheme && defined( 'SECRET_SALT' ) && SECRET_SALT && ( !isset($duplicated_keys[SECRET_SALT]) || empty($duplicated_keys[SECRET_SALT]) ) ) {
		...
		if ( in_array( $scheme, array( 'auth', 'secure_auth', 'logged_in', 'nonce' ), true ) ) {
			foreach ( array( 'key', 'salt' ) as $type ) {
				...
				if ( defined( $const ) && constant( $const ) && ( !isset($duplicated_keys[constant($const)]) || empty($duplicated_keys[constant($const)]) ) ) {
