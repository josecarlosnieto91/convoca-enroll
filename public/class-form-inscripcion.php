...
class Form_Inscripcion
{
     ...
     */
    public function render_form(int $actividad_id, array $attrs = []): string
    {
        if (!$actividad_id) {
            return '';
        }
        ...
        wp_enqueue_script('bde-public', BDE_URL . 'assets/js/biodevas-enroll-public.js', ['biodevas-common-js'], BDE_VERSION, true);
        wp_localize_script('bde-public', 'bdeEnroll', [
    ...
    public function handle_ajax(): void
    {
        ...
        if (!\Convoca\Core\Utils::check_rate_limit('inscribir', 5, 3600)) {
            wp_send_json_error(['errors' => ['Demasiados intentos de inscripción. Inténtalo de nuevo en una hora.']], 429);
        }
        ...
        $result = Motor_Inscripcion::inscribir($actividad_id, [
            'nombre' => $nombre,
            'email' => $email,
            'telefono' => $telefono,
            'dni' => $dni,
        ...
        if ($requiere_pago && $amount_eur > 0) {
            ...
            if (get_transient($lock_key)) {
                wp_send_json_error(['errors' => ['Ya hay un proceso de pago en curso para esta inscripción. Por favor, espera un momento.']], 429);
                return;
            }
