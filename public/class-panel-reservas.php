...
class Panel_Reservas
{
    ...
    public function __construct()
    {
        ...
        add_action('wp_ajax_nopriv_bde_panel_cancelar', [$this, 'ajax_cancelar']);
    }
    ...
    public function ajax_login(): void
    {
        ...
        if (!\Convoca\Core\Utils::check_rate_limit('panel_login', 10, 600)) {
            wp_send_json_error(['message' => __('Demasiados intentos. Inténtalo de nuevo en 10 minutos.', 'convoca-enroll')], 429);
        }
        ...
        if (!is_email($email)) {
            wp_send_json_error(['message' => __('Introduce un email válido.', 'convoca-enroll')]);
        }

        if (empty($codigo)) {
            wp_send_json_error(['message' => __('Introduce tu código de reserva.', 'convoca-enroll')]);
        }

        // Check if email + code match any inscription.
        $match = Motor_Inscripcion::buscar_por_codigo($email, $codigo);
        if (!$match) {
            wp_send_json_error(['message' => __('No se encontró ninguna reserva con ese email y código. Revisa los datos e inténtalo de nuevo.', 'convoca-enroll')]);
        }
    ...
    public function ajax_cancelar(): void
    {
        ...
        if (!\Convoca\Core\Utils::check_rate_limit('panel_cancelar', 5, 3600)) {
            wp_send_json_error(['message' => __('Demasiados intentos de cancelación. Inténtalo de nuevo en una hora.', 'convoca-enroll')], 429);
        }
        ...
        if (!is_email($email) || empty($codigo)) {
            wp_send_json_error(['message' => __('Datos de sesión no válidos.', 'convoca-enroll')]);
        }
        ...
        if (strtolower($email_inscripcion) !== strtolower($email) || $codigo_inscripcion !== $codigo) {
            wp_send_json_error(['message' => __('No tienes permiso para cancelar esta reserva o el código es incorrecto.', 'convoca-enroll')]);
        }

        // Verify inscription exists and is not already cancelled.
        $estado = CPT_Inscripcion::get_meta($inscripcion_id, 'estado');
        if ($estado === 'cancelada') {
            wp_send_json_error(['message' => __('Esta reserva ya está cancelada.', 'convoca-enroll')]);
        }
    ...
    private function get_reservas_formateadas(string $email): array
    {
        ...
        foreach ($posts as $post) {
            ...
            $reservas[] = [
                ...
                'cancelable'    => !in_array($estado, ['cancelada'], true),
