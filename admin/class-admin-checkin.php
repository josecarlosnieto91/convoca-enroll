...
class Admin_Checkin
{
    ...
    public static function render(): void
    {
        ...
        if ($actividad_id && !current_user_can('manage_options')) {
            $allowed_ids = CPT_Actividad::get_allowed_activities_ids();
            if (!in_array($actividad_id, $allowed_ids ?: [], true)) {
                wp_die(
        ...
        if ($actividad_id) {
            ...
            if ($search) {
                ...
                $query_args['meta_query'][] = [
                    'relation' => 'OR',
                    ['key' => '_bde_nombre', 'value' => $search, 'compare' => 'LIKE'],
                    ['key' => '_bde_email', 'value' => $search, 'compare' => 'LIKE'],
                ];
            }
            ...
            <?php if ($actividad_id) : ?>
                <div class="bde-checkin-stats">
                    ...
                    foreach ($inscripciones as $ins) {
                        $asistencia = get_post_meta($ins->ID, '_bde_asistencia', true);
                        if ($asistencia === 'si') {
                            $presentes++;
                        }
                    }
                    ...
                    <tbody>
                        ...
                        <?php else : ?>
                            <?php foreach ($inscripciones as $ins) : 
                                ...
                                $pagado = get_post_meta($id, '_bde_pagado', true);
                                $es_socio = get_post_meta($id, '_bde_es_socio', true);
                                $telefono = get_post_meta($id, '_bde_telefono', true);
                            ?>
