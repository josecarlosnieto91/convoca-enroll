...
body{position:relative;font-family:'Playfair',Georgia,serif;color:#fff;background:#2d1b4e;page-break-inside:avoid;break-inside:avoid}
.hero{position:absolute;top:0;left:0;width:100%;height:50%;overflow:hidden;background:#2d1b4e}
<?php if(!empty($hero_image)): ?>.hero{background-image:url("<?= $hero_image ?>");background-size:cover;background-position:center}
<?php endif; ?>
.solid{position:absolute;bottom:0;left:0;width:100%;height:50%;background:#2d1b4e;display:block}
.badge{position:absolute;top:80px;left:80px;display:inline-block;padding:8px 20px;font-family:'Montserrat',sans-serif;font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:2.5px;background:#7c4dff;color:#fff;z-index:1000}
...
.meta-item.price{background:#7c4dff;font-weight:700;color:#fff}
