...
class CPT_Actividad {
	...
	public function render_metabox( \WP_Post $post ): void {
		...
		<div class="convoca-grid-2">
			...
			<div class="convoca-field" style="grid-column: 1 / -1; margin-top: 0.5rem; padding-top: 1rem; border-top: 1px solid var(--conv-border, #ddd);">
				...
				$users                = get_users(
					array(
						'role__in' => array( 'administrator', 'editor', 'author', 'monitor_actividad' ),
	...
	public function save_metabox( int $post_id ): void {
		...
		$fields = array(
			'fecha_inicio'         => function ( $v ) {
				return str_replace( 'T', ' ', sanitize_text_field( $v ) ); },
			'fecha_fin'            => function ( $v ) {
				return str_replace( 'T', ' ', sanitize_text_field( $v ) ); },
	...
	public static function get_allowed_activities_ids(): ?array {
		...
		return ! empty( $ids ) ? array_map( 'intval', $ids ) : array( 0 );
	}
	...
	public function map_monitor_caps( array $caps, string $cap, int $user_id, array $args ): array {
		if (
			! in_array( $cap, array( 'edit_post', 'delete_post', 'read_post' ), true ) ||
			empty( $args ) ||
		...
		if ( $is_assigned ) {
			switch ( $cap ) {
				case 'edit_post':
				case 'read_post':
					return array( 'manage_inscripciones' ); // Grant permission if they are assigned.
				case 'delete_post':
					return array( 'do_not_allow' ); // Don't allow deletion even if they are assigned.
			}
		}
