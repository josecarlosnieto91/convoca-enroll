...
class Block_Inscripcion
{
    ...
    public function register_blocks(): void
    {
        ...
        wp_register_script(
            ...
            ['wp-blocks', 'wp-element', 'wp-server-side-render', 'wp-block-editor', 'wp-components', 'wp-i18n', 'wp-api-fetch'],
            BDE_VERSION,
            true
        );

        // 1. Formulario de Inscripción (existing)
        register_block_type(BDE_DIR . 'blocks/inscripcion', [
            'apiVersion'      => 3,
            'render_callback' => [$this, 'render_inscripcion'],
        ]);

        // 2. Panel de Reservas
        register_block_type(BDE_DIR . 'blocks/panel-reservas', [
            'apiVersion'      => 3,
            'render_callback' => [$this, 'render_panel_reservas'],
        ]);

        // 3. Página de Inscripciones
        register_block_type(BDE_DIR . 'blocks/pagina-inscripcion', [
            'apiVersion'      => 3,
            'render_callback' => [$this, 'render_pagina_inscripcion'],
        ]);

        // 4. Formulario de Evaluación
        register_block_type(BDE_DIR . 'blocks/evaluacion', [
            'apiVersion'      => 3,
            'render_callback' => [$this, 'render_evaluacion'],
            'attributes'      => [
        ...
        // 5. Lista de Espera
        register_block_type('biodevas-enroll/lista-espera', [
            'apiVersion'      => 3,
            'render_callback' => [$this, 'render_lista_espera'],
            'attributes'      => [
                'actividadId' => ['type' => 'number', 'default' => 0],
            ],
        ]);
