...
class Admin_Monitor_CRM
{
    public function __construct()
    {
        ...
        add_action('wp_ajax_bde_mark_attendance', [$this, 'ajax_mark_attendance']);
    }
    ...
    public function handle_actions(): void
    {
        ...
        wp_redirect(add_query_arg(['page' => 'bde-monitor-crm', 'actividad_id' => $actividad_id], admin_url('admin.php')));
        exit;
    }
    ...
    public function handle_exports(): void
    {
        ...
        if ($type === 'asistentes') {
            $args['meta_query'][] = ['key' => CPT_Inscripcion::META_PREFIX . 'asistencia', 'value' => '1'];
        }
    ...
    private function render_activities_list(): void
    {
        ...
        foreach ($activities as $act) {
            ...
            // Waitlist count
            $espera = (int) $wpdb->get_var($wpdb->prepare(
                ...
                CPT_Inscripcion::META_PREFIX . 'estado', 'lista_espera',
                CPT_Inscripcion::META_PREFIX . 'actividad_id', $act->ID
            ));
    ...
    private function render_activity_panel(int $actividad_id): void
    {
        ...
        echo '<a href="' . esc_url(remove_query_arg(['actividad_id', 'checkin'])) . '" class="bde-btn-premium" style="background: #fff; border: 1px solid var(--bde-border); color: var(--bde-text);">&larr; ' . esc_html__('Volver', 'convoca-enroll') . '</a>';
        if ($checkin_mode) {
        ...
        $export_all = wp_nonce_url(add_query_arg(['export' => 'todas', 'actividad_id' => $actividad_id]), 'bde_export_csv');
        $export_asist = wp_nonce_url(add_query_arg(['export' => 'asistentes', 'actividad_id' => $actividad_id]), 'bde_export_csv');
        ...
        $inscripciones = get_posts([
            'post_type' => 'inscripcion',
            'posts_per_page' => -1,
            'meta_query' => [['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => $actividad_id]]
        ]);
        ...
        foreach ($inscripciones as $ins) {
            ...
            if ($estado === 'confirmada') {
                $confirmadas++;
                $list_c[] = $ins;
                if ($asistencia === '1') $asistieron++;
                elseif ($asistencia === '0') $faltaron++;
                else $no_marcados++;
            } elseif (in_array($estado, ['pendiente', 'pendiente_pago'])) {
                $pendientes++;
                $list_p[] = $ins;
    ...
    private function render_table(array $items, int $actividad_id, bool $is_waitlist): void
    {
        ...
        if ($is_waitlist) echo '<th>' . esc_html__('Posición', 'convoca-enroll') . '</th>';
        if (!$is_waitlist) echo '<th style="text-align:center;">' . esc_html__('Check-in', 'convoca-enroll') . '</th>';
        ...
        foreach ($items as $item) {
            ...
            if ($estado !== 'confirmada') {
                ...
                $confirm_url = wp_nonce_url(add_query_arg(['action' => $confirm_action, 'id' => $item->ID, 'actividad_id' => $actividad_id]), 'bde_crm_' . $confirm_action . '_' . $item->ID);
