...
class Maintenance
{
    public function __construct() {
        add_action('bde_daily_maintenance', [__CLASS__, 'validar_integridad']);
        add_action('bde_daily_maintenance', [__CLASS__, 'reparar_integridad']);
    ...
    public static function validar_integridad(): array
    {
        ...
        foreach ($activities as $act_id) {
            ...
            ", CPT_Inscripcion::META_PREFIX . 'actividad_id', $act_id, CPT_Inscripcion::META_PREFIX . 'estado'));
    ...
    {
        global $wpdb;
        $stats = ['orphans_deleted' => 0, 'activities_recounted' => 0, 'logs_cleaned' => 0];

        // 1. Delete orphan inscriptions (only if activity is missing).
        $inscriptions = get_posts(['post_type' => 'inscripcion', 'posts_per_page' => -1, 'fields' => 'ids']);
        foreach ($inscriptions as $id) {
            $act_id = CPT_Inscripcion::get_meta($id, 'actividad_id');
            if (!$act_id || !get_post($act_id)) {
        ...
        $activities = get_posts(['post_type' => 'actividad', 'posts_per_page' => -1, 'fields' => 'ids']);
        foreach ($activities as $act_id) {
            $real_count = (int) $wpdb->get_var($wpdb->prepare("
                SELECT COUNT(*) FROM {$wpdb->postmeta} pm
                JOIN {$wpdb->posts} p ON p.ID = pm.post_id
        ...
        \Convoca\Core\Logger::log(
            sprintf('Reparación completada: %d huérfanos borrados, %d actividades recontadas, %d logs antiguos eliminados.', 
                $stats['orphans_deleted'], $stats['activities_recounted'], $stats['logs_cleaned']),
            'success',
            'Enroll/Maintenance'
        );
