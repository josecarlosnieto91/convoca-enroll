...
class Email_Queue {
	...
	private const TABLE_NAME = 'convoca_enroll_email_queue';
	...
	public function process_queue(): void {
		...
		if ( ! \Convoca\Core\Utils::acquire_lock( 'convoca_enroll_email_queue_lock', 300 ) ) {
			return;
		}
		...
		try {
			...
			// Rate limiting check.
			$hour_key      = 'convoca_enroll_email_hourly_count_' . gmdate( 'YmdH' );
			...
			foreach ( $emails as $email ) {
				...
				if ( microtime( true ) - $last_heartbeat > $heartbeat_interval ) {
					\Convoca\Core\Utils::release_lock( 'convoca_enroll_email_queue_lock' );
					\Convoca\Core\Utils::acquire_lock( 'convoca_enroll_email_queue_lock', 300 );
