...
class Motor_Inscripcion
{
    ...
    public static function inscribir(int $actividad_id, array $datos): int|\WP_Error
    {
        ...
        if ($is_member && $member_data) {
            ...
            $dni = strtoupper(str_replace([' ', '-'], '', sanitize_text_field($datos['dni'] ?? '')));
        ...
        $dup_query = [
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => $actividad_id],
                ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => $email],
                ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => ['cancelada'], 'compare' => 'NOT IN'],
            ],
        ];

        // If registering a minor, also match participant name to avoid true duplicates.
        if ($es_menor && !empty($nombre_participante)) {
            $dup_query['meta_query'][] = ['key' => CPT_Inscripcion::META_PREFIX . 'nombre_participante', 'value' => $nombre_participante];
        }
        ...
        if ($bloquear_dni === '1' && !empty($dni)) {
            $dni_query = [
                ...
                'meta_query' => [
                    'relation' => 'AND',
                    ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => $actividad_id],
                    ['key' => CPT_Inscripcion::META_PREFIX . 'dni', 'value' => $dni],
                    ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => ['cancelada'], 'compare' => 'NOT IN'],
                ],
            ];
        ...
        try {
            ...
            $estados_que_consumen_plaza = ['pendiente', 'pendiente_pago'];
    ...
    public static function confirmar(int $inscripcion_id): bool|\WP_Error
    {
        ...
        if (!in_array($estado_actual, ['pendiente', 'lista_espera', 'pendiente_pago'], true)) {
    ...
    private static function promote_waitlist(int $actividad_id): bool
    {
        $waitlist = get_posts([
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => $actividad_id],
                ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => 'lista_espera'],
            ],
        ]);
    ...
    public static function check_limite_reservas(string $email): true|\WP_Error
    {
        $existing = get_posts([
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => $email],
                ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => ['confirmada', 'pagada', 'pendiente', 'pendiente_pago', 'lista_espera'], 'compare' => 'IN'],
                [
                    'relation' => 'OR',
                    ['key' => CPT_Inscripcion::META_PREFIX . 'es_menor', 'value' => '1', 'compare' => '!='],
                    ['key' => CPT_Inscripcion::META_PREFIX . 'es_menor', 'compare' => 'NOT EXISTS'],
                ],
            ],
        ]);
    ...
    public static function generar_codigo(int $attempt = 0): string
    {
        ...
        $exists = get_posts([
            ...
            'posts_per_page' => 1,
            'post_status' => 'any',
            'meta_query' => [['key' => CPT_Inscripcion::META_PREFIX . 'codigo_reserva', 'value' => $code]],
        ]);
    ...
    public static function buscar_por_codigo(string $email, string $codigo): ?\WP_Post
    {
        $posts = get_posts([
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => sanitize_email($email)],
                ['key' => CPT_Inscripcion::META_PREFIX . 'codigo_reserva', 'value' => strtoupper(sanitize_text_field($codigo))],
            ],
        ]);
    ...
    public static function get_reservas_por_email(string $email): array
    {
        return get_posts([
            ...
            'meta_query' => [
                'relation' => 'AND',
                ['key' => CPT_Inscripcion::META_PREFIX . 'email', 'value' => sanitize_email($email)],
                ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => 'cancelada', 'compare' => '!='],
            ],
        ]);
    }
    ...
    public static function set_asistencia(int $inscripcion_id, string $asistencia): bool|\WP_Error
    {
        ...
        $valid_states = ['si', 'no', 'no_registrada'];
