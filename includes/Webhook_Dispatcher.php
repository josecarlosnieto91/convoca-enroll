...
class Webhook_Dispatcher {
	...
	public function __construct() {
		...
		add_action( 'convoca_enroll_process_webhook_queue', array( $this, 'process_queue' ) );

		// Listeners for state changes.
		add_action( 'convoca_enroll_inscripcion_nueva', array( $this, 'on_inscripcion_state_change' ), 10, 3 );
		add_action( 'convoca_enroll_inscripcion_cancelada', array( $this, 'on_inscripcion_state_change' ), 10, 2 );
		add_action( 'convoca_enroll_inscripcion_confirmada', array( $this, 'on_inscripcion_state_change' ), 10, 2 );
		add_action( 'convoca_enroll_inscripcion_promovida', array( $this, 'on_inscripcion_state_change' ), 10, 2 );
		add_action( 'convoca_enroll_asistencia_cambiada', array( $this, 'on_asistencia_cambiada' ), 10, 2 );
	}
				...
				try {
					...
					if ( ! $found ) {
						$wm::add_webhook(
							array(
								'url'    => $webhook_url,
								'events' => array( "enroll_$event_type" ),
