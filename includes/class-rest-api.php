...
class Rest_API
{
    ...
    {
        /* Public: activities */
        register_rest_route(self::NS, '/actividades', [
            'methods' => 'GET',
            'callback' => [$this, 'list_activities'],
        ...
        register_rest_route(self::NS, '/actividades/(?P<id>\d+)', [
            'methods' => 'GET',
            'callback' => [$this, 'get_activity'],
            'permission_callback' => '__return_true',
        ]);

        register_rest_route(self::NS, '/me/session-status', [
            'methods' => 'GET',
        ...
        /* Admin: inscriptions */
        register_rest_route(self::NS, '/inscripciones', [
            'methods' => 'GET',
        ...
        register_rest_route(self::NS, '/inscripciones/(?P<id>\d+)', [
            'methods' => 'PATCH',
            'callback' => [$this, 'update_inscription'],
            'permission_callback' => fn() => current_user_can('manage_inscripciones'),
            'args' => [
                'estado' => ['type' => 'string'],
        ...
        /* Admin: stats */
        register_rest_route(self::NS, '/stats/(?P<actividad_id>\d+)', [
            'methods' => 'GET',
            'callback' => [$this, 'get_stats'],
            'permission_callback' => fn() => current_user_can('manage_inscripciones'),
        ]);

        register_rest_route(self::NS, '/checkin', [
            'methods' => 'POST',
        ...
        register_rest_route(self::NS, '/ics', [
            'methods' => 'GET',
            'callback' => [$this, 'download_ics'],
            'permission_callback' => '__return_true',
            'args' => [
        ...
        register_rest_route(self::NS, '/admin/users/search', [
            'methods' => 'GET',
            'callback' => [$this, 'admin_search_users'],
            'permission_callback' => fn() => current_user_can('manage_inscripciones'),
            'args' => ['term' => ['type' => 'string', 'required' => true]],
        ]);
    }
    ...
    {
        if (!\Convoca\Core\Utils::check_rate_limit('bde_list_activities', 30, 60)) {
            return new \WP_REST_Response(['error' => __('Demasiadas peticiones.', 'convoca-enroll')], 429);
        }
    ...
    {
        if (!\Convoca\Core\Utils::check_rate_limit('bde_get_activity', 30, 60)) {
            return new \WP_REST_Response(['error' => __('Demasiadas peticiones.', 'convoca-enroll')], 429);
        }
        $id = (int) $req['id'];
        $post = get_post($id);
        if (!$post || $post->post_type !== 'actividad') {
            return new \WP_Error('not_found', __('Actividad no encontrada.', 'convoca-enroll'), ['status' => 404]);
        }
    ...
    public function list_inscriptions(\WP_REST_Request $req): \WP_REST_Response
    {
        ...
        if ($req->get_param('actividad_id')) {
            $meta_query[] = ['key' => CPT_Inscripcion::META_PREFIX . 'actividad_id', 'value' => (int) $req['actividad_id']];
        }
        if ($req->get_param('estado')) {
            $meta_query[] = ['key' => CPT_Inscripcion::META_PREFIX . 'estado', 'value' => sanitize_text_field($req['estado'])];
        }
        if ($meta_query) {
        ...
        if (!current_user_can('manage_options')) {
            ...
            $items = array_filter($items, fn($item) => in_array($item['actividad_id'], $allowed_ids ?? [], true));
        }

        return new \WP_REST_Response(['items' => array_values($items), 'total' => count($items)]);
    }
    ...
    public function update_inscription(\WP_REST_Request $req): \WP_REST_Response|\WP_Error
    {
        ...
        if (!$this->check_permission_for_activity($act_id)) {
            return new \WP_Error('rest_forbidden', 'No tienes permiso para gestionar esta actividad.', ['status' => 403]);
        }
    ...
    public function get_stats(\WP_REST_Request $req): \WP_REST_Response|\WP_Error
    {
        ...
        if (!$this->check_permission_for_activity($act_id)) {
            return new \WP_Error('rest_forbidden', 'No tienes permiso para ver estadísticas de esta actividad.', ['status' => 403]);
        }
    ...
    public function handle_checkin(\WP_REST_Request $request): \WP_REST_Response
    {
        ...
        $posts = get_posts([
            'post_type' => 'inscripcion',
            'meta_query' => [
                ['key' => CPT_Inscripcion::META_PREFIX . 'checkin_token', 'value' => $token]
            ],
            'posts_per_page' => 1,
            'post_status' => 'publish',
        ]);
        ...
        // Check permission
        if (!$this->check_permission_for_activity($act_id)) {
            \Convoca\Core\Logger::info("Intento de check-in fallido: Usuario sin permisos para actividad #$act_id", 'Enroll/Checkin', $monitor_id, ['inscripcion_id' => $insc_id]);
            return new \WP_REST_Response(['error' => 'No tienes permiso para realizar check-in en esta actividad.'], 403);
        }
        ...
        if ($affected > 0) {
            \Convoca\Core\Logger::info("Check-in exitoso vía QR por usuario #$monitor_id", 'Enroll/Checkin', $monitor_id, ['inscripcion_id' => $insc_id]);
        } else {
            \Convoca\Core\Logger::info("Escaneo de QR repetido por usuario #$monitor_id", 'Enroll/Checkin', $monitor_id, ['inscripcion_id' => $insc_id]);
        }
    ...
    public function admin_search_users(\WP_REST_Request $req): \WP_REST_Response
    {
        ...
        if (strlen($term) < 2) return new \WP_REST_Response([], 200);

        $users = get_users([
            'role__in' => ['administrator', 'editor', 'monitor_actividad', 'monitor'],
            'search' => '*' . $term . '*',
