...
class Google_Calendar
{
    ...
    public function __construct()
    {
        ...
        // Hooks for sync.
        add_action('save_post_actividad', [$this, 'sync_on_save'], 20, 3);
        add_action('before_delete_post', [$this, 'sync_on_delete'], 10, 2);
    }
    ...
    public function sync_on_save(int $post_id, \WP_Post $post, bool $update): void
    {
        ...
        if ($sync_enabled === '') {
            $settings = get_option(self::OPTION, []);
            if (empty($settings['google_calendar_auto_sync'])) return;
        }
    ...
    public function generate_ics(int $actividad_id): ?string
    {
        ...
        try {
            ...
            $vCalendar = new ICalCalendar([$vEvent]);
