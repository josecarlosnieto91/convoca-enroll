...
class Checkin_Handler
{
    public function __construct()
    {
        ...
        add_action('template_redirect', [$this, 'handle_checkin_page']);
        add_action('wp_ajax_bde_qr_checkin', [$this, 'ajax_qr_checkin']);
    }
    ...
    public function handle_checkin_page(): void
    {
        ...
        if (!current_user_can('manage_inscripciones') && !in_array('voluntario_aprobado', (array) wp_get_current_user()->roles, true)) {
            wp_die(__('No tienes permisos para realizar check-in.', 'convoca-enroll'), __('Acceso Denegado', 'convoca-enroll'), ['response' => 403]);
        }
    ...
    private function process_direct_checkin(string $token): void
    {
        ...
        $inscriptions = get_posts(['post_type' => 'inscripcion', 'meta_key' => '_bde_checkin_token', 'meta_value' => $token, 'posts_per_page' => 1]);
        ...
        wp_die(
            sprintf(__('Check-in confirmado para %s en "%s".', 'convoca-enroll'), '<strong>' . esc_html($nombre) . '</strong>', esc_html($act_title)),
    ...
    public function ajax_qr_checkin(): void
    {
        ...
        if (!CPT_Actividad::is_user_responsible($user_id, $act_id)) {
             \Convoca\Core\Logger::info("Intento de check-in fallido (AJAX): Sin permisos para actividad #$act_id", 'Enroll/Checkin', $user_id, ['inscripcion_id' => $id]);
             wp_send_json_error(__('No tienes permiso para gestionar esta actividad.', 'convoca-enroll'));
        }

        \Convoca\Core\Logger::info("Check-in exitoso vía QR (AJAX) por usuario #$user_id", 'Enroll/Checkin', $user_id, ['inscripcion_id' => $id]);
