...
class CPT_Inscripcion
{
    ...
    public function __construct()
    {
        ...
        add_action('load-post-new.php', [__CLASS__, 'redirect_default_editor']);
        add_action('load-post.php', [__CLASS__, 'redirect_default_editor']);
        add_action('save_post_inscripcion', [__CLASS__, 'on_save_link_member'], 10, 3);
    }
    ...
    public static function get_meta(int $post_id, string $key, bool $single = true)
    {
        ...
        if ($key === 'asistencia') {
            if ($val === '1') return 'si';
            if ($val === '0') return 'no';
            if (empty($val)) return 'no_registrada';
        }
        return $val;
    }
